...
const ListingDetailPage = () => {
    ...
    if (!listing) {
        return (
            <div style={{ padding: '200px', textAlign: 'center', color: 'var(--text-white)' }}>
                <h2>매물을 찾을 수 없습니다.</h2>
                ...
                <div className="container">
                    <div style={{ display: 'flex', gap: '50px', alignItems: 'flex-start', flexWrap: 'wrap' }}>
                        ...
                        <div style={{ flex: 2, minWidth: '300px' }}>
                            {/* Tabs */}
                            <div style={{ display: 'flex', borderBottom: '1px solid var(--glass-border)', marginBottom: '40px' }}>
                                {tabs.map(tab => (
                                    <button
                                        key={tab.id}
                            ...
                            <AnimatePresence mode='wait'>
                                ...
                                >
                                    ...
                                    {activeTab === 'analysis' && (
                                        <div>
                                            ...
                                            <div className="glass-card" style={{ padding: '30px', marginTop: '20px' }}>
                                                ...
                                                <div style={{ background: 'var(--glass-bg)', padding: '20px', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', border: '1px solid var(--glass-border)' }}>
                                                    <div>
                                                        <div style={{ fontWeight: 'bold', color: 'var(--text-white)' }}>{listing.title} - 정밀 분석 리포트.pdf</div>
                                                        <div style={{ fontSize: '0.8rem', color: 'var(--text-gray)' }}>Generated by GAJA AI • 2.4MB</div>
                                                    </div>
                                    ...
                                    {activeTab === 'location' && (
                                        <div className="glass-card" style={{ padding: '40px', minHeight: '300px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                            <div style={{ textAlign: 'center' }}>
                                                <i className="fas fa-map-marked-alt" style={{ fontSize: '3rem', color: 'var(--accent-gold)', marginBottom: '20px' }}></i>
                                                <p style={{ color: 'var(--text-off-white)' }}>상세 주소 및 지도는<br />보안상 <strong style={{ color: 'var(--text-white)' }}>컨설팅 신청 고객</strong>에게만 제공됩니다.</p>
                                            </div>
                                        </div>
                                    )}
                                </motion.div>
                        ...
                        <div style={{ flex: 1, minWidth: '300px', position: 'sticky', top: '100px' }}>
                            <div className="glass-card" style={{ padding: '30px', border: '1px solid var(--accent-gold)' }}>
                                ...
                                <ul style={{ marginBottom: '30px', fontSize: '0.9rem', color: 'var(--text-off-white)' }}>
...
const InfoItem = ({ label, value, highlight, icon }) => (
    <div style={{ display: 'flex', alignItems: 'center' }}>
        <div style={{ width: '50px', height: '50px', background: 'var(--glass-bg)', border: '1px solid var(--glass-border)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', marginRight: '15px', color: 'var(--accent-gold)' }}>
            <i className={`fas ${icon}`}></i>
        </div>
        <div>
            <div style={{ fontSize: '0.85rem', color: 'var(--text-gray)', marginBottom: '3px' }}>{label}</div>
            <div style={{ fontSize: '1.2rem', fontWeight: '700', color: highlight ? 'var(--accent-gold)' : 'var(--text-white)' }}>{value}</div>
        </div>
    </div>
);
